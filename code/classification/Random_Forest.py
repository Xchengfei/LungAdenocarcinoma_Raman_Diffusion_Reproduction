...
def load_split(split_dir: Path, split_name: str) -> tuple[pd.DataFrame, np.ndarray, np.ndarray, list[str]]:
...
def grid_search_random_forest(
    ...
    if best_model is None or best_row is None:
        raise RuntimeError("Random forest grid search did not train any model")
...
def save_confusion_matrix(y_true: np.ndarray, y_pred: np.ndarray, output_path: Path) -> None:
    ...
    ax.set_title("Random forest confusion matrix (test set)", pad=6)
...
def save_roc_curve(y_true: np.ndarray, y_prob: np.ndarray, output_path: Path) -> None:
    ...
    ax.set_title("Random forest ROC curve (test set)", pad=6)
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1.02)
...
def save_probability_distribution(y_true: np.ndarray, y_prob: np.ndarray, output_path: Path) -> None:
    ...
    ax.set_title("Random forest probability distribution (test set)", pad=6)
...
def save_pca_predictions(
    ...
    ax.set_title("Random forest test predictions in PCA space", pad=6)
...
def save_feature_importance(
    ...
    importance = pd.DataFrame(
        {
            "wavenumber": feature_columns,
            "importance": model.feature_importances_,
        }
    )
    importance["wavenumber_float"] = importance["wavenumber"].astype(float)
    ...
    ax.set_title(f"Top {top_n} random forest features", pad=6)
    style_axis(ax)
    fig.tight_layout()
    fig.savefig(output_path, bbox_inches="tight")
    plt.close(fig)
...
def main() -> None:
    ...
    figure_dir = root / "outputs" / "reports" / "figures" / "random_forest"
    table_dir.mkdir(parents=True, exist_ok=True)
    figure_dir.mkdir(parents=True, exist_ok=True)

    _, x_train, y_train, feature_columns = load_split(split_dir, "train")
    _, x_val, y_val, _ = load_split(split_dir, "val")
    _, x_test, y_test, _ = load_split(split_dir, "test")

    model, best_params, grid_results = grid_search_random_forest(x_train, y_train, x_val, y_val)
    grid_path = table_dir / "random_forest_grid_search.csv"
    ...
    save_confusion_matrix(y_test, test_pred, figure_dir / "random_forest_confusion_matrix_test.png")
    save_roc_curve(y_test, test_prob, figure_dir / "random_forest_roc_curve_test.png")
    save_probability_distribution(y_test, test_prob, figure_dir / "random_forest_probability_distribution_test.png")
    save_pca_predictions(x_train, x_test, y_test, test_pred, figure_dir / "random_forest_test_predictions.png")
    save_feature_importance(
        model,
        feature_columns,
        figure_dir / "random_forest_top_features.png",
    ...
    summary = {
        ...
        "feature_importance": str(table_dir / "random_forest_feature_importance.csv"),
